...
규칙 넷은 실패 사례에서 거꾸로 뽑았다. 셋이 서로 다른 징후를 냈기 때문이다.
  ...
  C2  새로 세운 주체의 기관 이름이 문서에 없다          -> 지어냄
  C3  새 수치가 문서 어디에도 없다                    -> 지어냄
  C4  다른 조항을 받는 조항에서 주체를 통째로 갈았다      -> 참조가 깨진다

**하드 게이트가 아니라 표시다. 버킷도 가르지 않는다.** 사람이 판정한 표본이 10건뿐이라
문턱을 세울 근거가 없다. `docs/기획서_최종.md` 2.3절이 200건마다 10건을 요구하는데, 그
10건을 무작위로 뽑는 대신 여기 걸린 것부터 읽으면 같은 품으로 더 많이 잡는다 -- **읽을
순서를 정하는 데 쓰지, 버리거나 빼내는 데 쓰지 않는다.**

**두 규칙의 오탐이 2026-08-12에 드러났다.** 같은 60건에서 C2 15건 중 8건, C4 5건 중
4건이 오탐이었다.

- **C2는 고쳤다.** `ACTOR`가 주체를 `의 장`까지 붙여 뽑는데 그 통짜 문자열로 문서를
  찾고 있었다. `공동연구개발기관`이 문서에 있어도 `공동연구개발기관의 장`은 없어서
  정상 표현이 지어냄으로 잡혔다. 지금은 `의 장`을 떼고 기관 이름으로 대조한다
- **C4는 아직이다.** 둘은 `SUBJ`가 용언 어간을 주어로 오인한 것이고(`수행하려`는,
  `관리하`는), 둘은 `(수행 주체, 다른 값)` 지시에 걸렸다 -- **주체를 바꾸라고 시켜놓고
  주체가 바뀌었다고 벌준다.** 지시가 `수행 주체`일 때는 걸지 않는 것이 맞아 보이나,
...
def inspect(clause: str, after: str, target: str, document: str) -> list[str]:
    ...
    #
    # **`의 장`을 떼고 기관 이름으로 대조한다.** `ACTOR`가 주체를 `의 장`까지 붙여 뽑으므로
    # 통짜로 찾으면 문서에 있는 기관의 장까지 지어냄으로 잡힌다 -- `공동연구개발기관`이
    # 문서에 있어도 `공동연구개발기관의 장`은 없기 때문이다. 문서에 있는 기관의 장을
    # 가리키는 것은 법령체에서 정상 표현이다. 2026-08-12 60건에서 C2 15건 중 8건이 이
    # 오탐이었고, 걷어내니 BM7이 70.0%에서 81.7%가 됐다.
    for word in sorted(set(ACTOR.findall(after)) - set(ACTOR.findall(clause))):
        if re.sub(r"의\s*장$", "", word) not in document:
